...
class ServiceBusUtil:
    ...
    async def _send_message(
        ...
        async with ServiceBusClient(
            ...
            try:
                print(f"sumitting message to topic '{topic_name}'")
                # Get a Topic Sender object to send messages to the topic
                sender = service_bus_client.get_topic_sender(topic_name)
                async with sender:
                    message = ServiceBusMessage(
                        json.dumps(payload), subject=service_name
                    )
    ...
    def send_redaction_process_complete_message(
        ...
        """
        Send a message to the `redaction_process_complete` topic on the service bus
        """
        return asyncio.run(
            self._send_message("redaction_process_complete", pins_service, payload)
        )
