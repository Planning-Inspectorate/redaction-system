...
resource "azurerm_storage_account" "redaction_storage" {
  ...
  name                             = "${local.org}st${local.service_name}${var.environment}${local.location_short}"
...
resource "azurerm_cognitive_account" "open_ai" {
  ...
  local_auth_enabled                 = false # Temp for now - need some permissions in place to do this
  identity {
    type = "SystemAssigned"
  }
}
